...
geno <- read_geno(filename=geno.file, ploidy=4, map=TRUE, w=0.4, ped=ped)
...
CDF1 accounted for 12% of the phenotypic covariance, compared with 55% for the polygenic additive effect and 13% for the residual genetic effect. The "cor" matrix in the output shows the additive correlation (due to fixed and random effects) above the diagonal, and the total genotypic correlation is below the diagonal. 
...
index2 <- blup(prep, geno, what="id",
               index.weights=c(total.yield=1,vine.maturity=-0.5))
...
The above figure compares the rank of genotypes under the two different indices. The top 3 genotypes were the same under both indices, while the genotype ranked 15th for yield jumped to fourth place under the penalized index.  
