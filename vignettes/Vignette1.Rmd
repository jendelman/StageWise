...
ans1a <- Stage1(filename=pheno1a.file,traits="total.yield",
                effects=effects,solver="asreml",silent=FALSE)
```
...
model1 <- Stage1(filename=pheno1b.file, traits="total.yield",
                effects=effects, solver="asreml",silent=TRUE)
model1$fit

model2 <- Stage1(filename=pheno1b.file, traits="total.yield",silent=TRUE,
...
The `summary` function returns a matrix with information about the G and (when present) D matrices. Column 1 is the diagonal elements of G. Column 2 is the diagonal elements of D. Column 3 contains the genomic inbreeding coefficients $F_g$ from the dominance matrix, which have an expected value of 1 for homozygous lines. The population mean $\overline F_g$ represents the deviation from panmictic (i.e., Hardy-Weinberg) proportions, and its mathematical relationship to the mean of the diagonal elements of G is identical to the mathematical relationship between the diagonal elements of the pedigree relationship matrix and the inbreeding coefficient based on kinship. For ploidy $\phi$, the formula is $\overline F_g = \frac{\overline G_{..} - 1}{\phi - 1}$. The result for $\overline F_g$ is negative for this tetraploid potato population, indicating there is excess heterozygosity, which may be expected when there is inbreeding depression. 
...
avg <- apply(x,2,mean)
avg
(as.numeric(avg[1])-1)/(4-1)  #equals mean Fg
```
...
The above Stage 2 analysis did not include the marker data. To partition the genotype effects into additive and non-additive effects, the output from `read_geno` is included in the function call. StageWise has two ways of modeling non-additive effects. The option `dominance=FALSE` leads to a residual genetic model, with independent and identically distributed (iid) effects. When `dominance=TRUE` (which requires that `read_geno` was run with dominance=TRUE), the covariance of the non-additive effects follows the D matrix. The AIC can be used to assess which model is better.
...
When dominance is not included, the variance table has rows for "additive" and "genotype", which is the iid residual genetic effect. The output for the dominance model includes rows for "dominance" and "heterosis"; the former is based on the variance of a random effect with zero mean, while the latter is due to the mean ([Varona et al. 2018](https://doi.org/10.3389/fgene.2018.00078)). The ability to estimate heterosis is limited by the lack of variation for inbreeding in the population.
...
```
```{r,eval=FALSE}
ans2e <- Stage2(data=stage1.blue,vcov=stage1.vcov,geno=geno2,dominance=TRUE)
```
```{r,include=FALSE}
load("~/Box Sync/Endelman/Software/StageWise/Vignette1.rda")
```
```{r}
ans2e$aic
...
ans2 <- vector("list",5)
for (i in 1:5) {
  ans2[[i]] <- Stage2(data=stage1.blue,vcov=stage1.vcov,geno=geno[[i]],dominance=TRUE)
...
One last point about `Stage2`: the function returns the fixed effect estimates under "fixed", and the predicted random effects for the individuals in the training set are returned under "random". 

### BLUP Reliability

The calculation of BLUPs is split into two functions: `blup_prep` and `blup`. The computationally intensive steps occur in `blup_prep`, which combines the phenotype and genotype information used in `Stage2` with the variance component estimates to estimate the var-cov matrix of the predicted random effects. The `blup` command is faster because it merely extracts the appropriate linear combination of predictions based on the argument "what", which has 5 possible values:
...
```{r}
prep1 <- blup_prep(data=stage1.blue,
              vcov=stage1.vcov,
              geno=genoH,
              vars=ans2H$vars)
...
prep3 <- blup_prep(data=stage1.blue,
              vcov=stage1.vcov,
              geno=genoH,
              vars=ans2H$vars,
              mask=mask)
...
```{r,fig.height=3,fig.width=3}
#w = 0
prep <- blup_prep(data=stage1.blue,vcov=stage1.vcov,geno=geno[[1]],vars=ans2[[1]]$vars)
...
ans2vm.1 <- Stage2(data=ans1vm$blue,
               vcov=ans1vm$vcov,
               geno=geno[[1]],
               fix.eff.marker="solcap_snp_c2_22964")
ans2vm.1$aic
```
The AIC decreased by 71 points when including the marker linked to *CDF1* as a fixed effect. 

```{r}
# Fixed effect for the marker
ans2vm.1$fixed$marker

# Proportion of variance
kable(summary(ans2vm.1$vars))
```

The fixed effect estimate of -0.78 for solcap_snp_c2_22964 implies that, on average, each additional copy of the alternate allele reduced vine maturity by 0.78 (on a 1-9 visual scale). According to the proportion of variance table, the marker accounted for 0.124/(0.124 + 0.234) = 35% of the breeding value.
