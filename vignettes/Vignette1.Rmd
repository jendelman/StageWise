...
opts_knit$set(root.dir="~/Box/Endelman/Software/StageWise")
```
...
The `Stage1` function returns a list with several results. List element "blue" is a data frame of the individual BLUEs per environment. Element "fit" contains the AIC and broad-sense heritability (H2), with H2 reported per plot (i.e., Vg/(Vg+Ve)) and entry-mean (i.e., generalized H2; Oakey et al. 2006). 
...
With SpATS, only the generalized (entry-mean) heritability is calculated. 
...
Before proceeding to Stage 2, the BLUEs and variance-covariance matrices from the 2015-19 analysis and 2020 analysis need to be combined. (If other software is used for Stage 1, it can be incorporated in a similar manner.) 
