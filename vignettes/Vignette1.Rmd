...
```{r}
prep1 <- blup_prep(data=ans1$blue,
              vcov=ans1$vcov,
              geno=geno,
              vars=ans2c$vars)
blup1 <- blup(prep1,geno=geno,what="id")
```
...
prep2 <- blup_prep(data=ans1$blue,
              vcov=ans1$vcov,
              geno=geno,
              vars=ans2c$vars,
              mask=mask)
blup2 <- blup(prep2,geno=geno,what="id")
...
Here is the code to calculate GWAS scores for vine maturity, which are returned in the second element of a list. `gwas_threshold` computes the -log10(p) threshold for QTL discovery based on an effective number of markers ([Moskvina and Schmidt, 2008](https://doi.org/10.1002/gepi.20331)), and `manhattan_plot` displays the result. 
...
### Stage 2 analysis with marker fixed effects
...
```{r}
k <- which.max(blup3[[2]]$gwas.score)
blup3[[2]][k,]

ans3 <- Stage2(data=ans1[["vine.maturity"]]$blue,
               vcov=ans1[["vine.maturity"]]$vcov,
               geno=geno,
